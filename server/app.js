...
async function ensureGuestSeedCards() {
  try {
    const card = await db.prepare("SELECT COUNT(*) as c FROM spaced_repetition_cards WHERE user_id = 'guest' AND item_type = 'flashcard'").get();
    const count = card?.c || 0;
    if (count === 0) {
      const flashcards = await db.prepare('SELECT id FROM flashcards').all();
      const nowIso = new Date().toISOString();
      const stmts = flashcards.map(fc => ({
 ...
 */
async function migrateGuestData(targetUserId) {
  if (!targetUserId || targetUserId === 'guest') return;
  try {
    // 1. user_lesson_progress
    const guestLessons = await db.prepare("SELECT * FROM user_lesson_progress WHERE user_id = 'guest'").all();
    const stmts = [];
    guestLessons.forEach(l => {
      stmts.push({
        ...
        args: [l.lesson_id, targetUserId, l.status, l.quick_checks_passed, l.completed_at, l.updated_at]
      });
    });
    stmts.push({ sql: "DELETE FROM user_lesson_progress WHERE user_id = 'guest'", args: [] });

    // 2. user_question_attempts
    stmts.push({ sql: "UPDATE user_question_attempts SET user_id = ? WHERE user_id = 'guest'", args: [targetUserId] });

    // 3. spaced_repetition_cards
    const guestCards = await db.prepare("SELECT * FROM spaced_repetition_cards WHERE user_id = 'guest'").all();
    guestCards.forEach(c => {
      ...
      stmts.push({
        sql: `INSERT INTO spaced_repetition_cards (id, user_id, item_type, item_id, repetition, interval_days, ease_factor, due_date, last_reviewed_at, last_rating)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
              ON CONFLICT(user_id, item_type, item_id) DO NOTHING`,
        args: [newCardId, targetUserId, c.item_type, c.item_id, c.repetition, c.interval_days, c.ease_factor, c.due_date, c.last_reviewed_at, c.last_rating]
      });
    });
    stmts.push({ sql: "DELETE FROM spaced_repetition_cards WHERE user_id = 'guest'", args: [] });

    // 4. mock_sessions
    stmts.push({ sql: "UPDATE mock_sessions SET user_id = ? WHERE user_id = 'guest'", args: [targetUserId] });

    // 5. study_settings
    const guestSettings = await db.prepare("SELECT * FROM study_settings WHERE user_id = 'guest'").all();
    guestSettings.forEach(s => {
      stmts.push({
        sql: `INSERT INTO study_settings (key, user_id, value)
              VALUES (?, ?, ?)
              ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value`,
        args: [s.key, targetUserId, s.value]
      });
    });
    stmts.push({ sql: "DELETE FROM study_settings WHERE user_id = 'guest'", args: [] });
...
// Auto-initialize DB schema and seeds on cold start
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const cleanUsername = username.trim();
    const user = await db.prepare('SELECT * FROM users WHERE LOWER(username) = LOWER(?)').get(cleanUsername);
    if (!user) {
...
app.post('/api/auth/migrate-guest', requireAuth, async (req, res) => {
  try {
    await migrateGuestData(req.userId);
...
app.post('/api/lessons/:lessonId/complete', async (req, res) => {
  try {
    ...
    const lesson = await db.prepare('SELECT * FROM lessons WHERE id = ?').get(req.params.lessonId);
    if (!lesson) {
      return res.status(404).json({ error: 'Lesson not found' });
    }
    ...
    if (allCorrect) {
      ...
      const existingSr = await db.prepare('SELECT * FROM spaced_repetition_cards WHERE user_id = ? AND item_type = ? AND item_id = ?').get(req.userId, 'lesson', lesson.id);
      if (!existingSr) {
...
app.post('/api/reviews/:cardId/rate', async (req, res) => {
  try {
    ...
    const card = await db.prepare('SELECT * FROM spaced_repetition_cards WHERE id = ? AND user_id = ?').get(req.params.cardId, req.userId);
    if (!card) {
...
// 9. Attempt Question
app.post('/api/questions/:questionId/attempt', async (req, res) => {
  try {
    const { userAnswer, timeSpentSecs = 30, mode = 'practice', sessionId = null } = req.body;
    const q = await db.prepare('SELECT * FROM questions WHERE id = ?').get(req.params.questionId);
    if (!q) {
    ...
    const existingSr = await db.prepare('SELECT * FROM spaced_repetition_cards WHERE user_id = ? AND item_type = ? AND item_id = ?').get(req.userId, 'question', q.id);
    if (!isCorrect) {
      const tomorrow = new Date();
      tomorrow.setDate(tomorrow.getDate() + 1);
      if (existingSr) {
        await db.prepare('UPDATE spaced_repetition_cards SET due_date = ?, repetition = 0 WHERE id = ? AND user_id = ?').run(tomorrow.toISOString(), existingSr.id, req.userId);
      } else {
        await db.prepare(`
          INSERT INTO spaced_repetition_cards (id, user_id, item_type, item_id, repetition, interval_days, ease_factor, due_date, last_reviewed_at, last_rating)
...
// 10. Weak Area Drill
app.get('/api/weak-areas/drill', async (req, res) => {
  try {
    const drillQuestions = await db.prepare(`
...
// 13. Mock History
app.get('/api/mock/history', async (req, res) => {
  try {
    const history = await db.prepare('SELECT * FROM mock_sessions WHERE user_id = ? ORDER BY created_at DESC').all(req.userId);
    history.forEach(h => {
...
app.get('/api/calendar', async (req, res) => {
  try {
    const rows = await db.prepare('SELECT * FROM study_settings WHERE user_id = ?').all(req.userId);
    const config = {};
    rows.forEach(r => {
      try {
  ...
  try {
    ...
    if (target_exam_date) {
      stmts.push({
        ...
        args: ['target_exam_date', req.userId, String(target_exam_date)]
      });
    }
    if (target_cutoff) {
      stmts.push({
    ...
    if (current_mode) {
      stmts.push({
        sql: 'INSERT INTO study_settings (key, user_id, value) VALUES (?, ?, ?) ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value',
        args: ['current_mode', req.userId, String(current_mode)]
      });
    }
    if (busy_periods) {
      stmts.push({
        sql: 'INSERT INTO study_settings (key, user_id, value) VALUES (?, ?, ?) ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value',
        args: ['busy_periods', req.userId, JSON.stringify(busy_periods)]
      });
    }
...
app.post('/api/backup/import', async (req, res) => {
  try {
    const result = await importDatabaseJSON(req.body);
    res.json(result);
  } catch (err) {
...
app.post('/api/backup/snapshot', async (req, res) => {
  try {
    const snapshotPath = await createLocalSnapshot();
...
// Explicit JSON 404 for any unhandled API endpoints
app.use('/api', (req, res) => {
