  ...
  try {
    if (Array.isArray(payload.user_lesson_progress)) {
      ...
      payload.user_lesson_progress.forEach(p => {
        stmt.run(p.lesson_id, p.status, p.quick_checks_passed, p.completed_at, p.updated_at);
      });
    }
    ...
    if (Array.isArray(payload.user_question_attempts)) {
      ...
      payload.user_question_attempts.forEach(a => {
        stmt.run(a.id, a.question_id, a.user_answer, a.is_correct, a.time_spent_secs, a.mode, a.session_id, a.attempted_at);
      });
    }
    ...
    if (Array.isArray(payload.spaced_repetition_cards)) {
      ...
      payload.spaced_repetition_cards.forEach(c => {
        stmt.run(c.id, c.item_type, c.item_id, c.repetition, c.interval_days, c.ease_factor, c.due_date, c.last_reviewed_at, c.last_rating);
      });
    }
    ...
    if (Array.isArray(payload.mock_sessions)) {
      ...
      payload.mock_sessions.forEach(m => {
        stmt.run(m.id, m.title, m.total_marks, m.score_obtained, m.target_cutoff, m.passed_cutoff, m.duration_seconds, m.time_spent_seconds, m.subject_breakdown, m.created_at);
      });
    }
    ...
    if (Array.isArray(payload.study_settings)) {
      ...
      const stmt = db.prepare('INSERT INTO study_settings (key, value) VALUES (?, ?)');
