    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4 overflow-y-auto">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-8 py-2 bg-slate-950 border rounded-xl text-sm text-slate-100 placeholder-slate-500 focus:outline-none ${
              ...
              />
              {/* Availability spinner / icon */}
              {mode === 'register' && username.trim().length >= 3 && (
                <span className="absolute right-3 top-1/2 -translate-y-1/2">
                  {isCheckingUsername ? (
                    <span className="w-3.5 h-3.5 border-2 border-slate-500 border-t-cyan-400 rounded-full animate-spin block" />
            ...
            {/* Availability hint */}
            {mode === 'register' && username.trim().length >= 3 && !isCheckingUsername && (
              usernameAvailable === true ? (
                ...
                <p className="text-[11px] text-rose-400 flex items-center gap-1">
                  <AlertCircle className="w-3 h-3" /> Username already taken. Try another.
                </p>
              ) : null
            )}
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading || ((mode === 'register' || mode === 'reset') && confirmPassword.length > 0 && password !== confirmPassword) || (mode === 'register' && usernameAvailable === false)}
